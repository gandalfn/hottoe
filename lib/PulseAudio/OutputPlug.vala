...
internal class SukaHottoe.PulseAudio.OutputPlug : Plug {
    ...
    public override bool is_muted {
        ...
        set {
            m_is_muted = value;
            ((Manager) manager).operations.set_sink_input_mute (index, m_is_muted);
        }
    }
    ...
    public OutputPlug (Manager in_manager, global::PulseAudio.SinkInputInfo in_info) {
        GLib.Object (
            manager: in_manager,
    ...
    public bool update (global::PulseAudio.SinkInputInfo in_info) {
        bool updated = false;
        ...
        if (m_channel == null || m_channel.index != in_info.sink) {
            foreach (var c in manager.get_output_channels ()) {
                if (((Channel) c).index == in_info.sink) {
                    channel = c;
                    notify_property ("channel");
                    updated = true;
                    break;
